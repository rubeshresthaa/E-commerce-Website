   ...
   if(isset($_POST['place_order'])){
      ...
      //3.get products from cart (from session)
       foreach($_SESSION['cart'] as $key=> $value){
         ...
         $produc_price=$product['product_price'];
         $product_quantity=$product['product_quantity'];
         //4.store each single item in order_items database

         $stmt1=$conn ->prepare("INSERT INTO order_items(order_id,product_id,product_name,product_image,product_price,product_quantity,user_id,order_date)
                          VALUES (?,?,?,?,?,?,?,?)");
         $stmt1 ->bind_param('iissiiis',$order_id,$product_id,$product_name,$product_image,$produc_price,$product_quantity,$user_id,$order_date);
         $stmt1->execute();                
       }
